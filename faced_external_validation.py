"""Run one frozen, participant-wise FACED fold for self-contained DRSR."""
...
STUDY = "faced_external_validation"
DESIGN_PATH = HERE / "faced_external_validation_design.json"
SMOKE_TAG = "faced_external_same_model_smoke_r3_20260909"
PRODUCTION_TAG = "faced_external_same_model_production_r3_20260909"
CHECKPOINTS = tuple(range(50, 1001, 50))
SOURCE_FILES = (
    "faced_external_validation.py",
    "faced_external_validation_design.json",
    "FACED_EXTERNAL_VALIDATION_CONTRACT.md",
...
def load_design() -> dict[str, Any]:
    ...
    if design.get("faced_performance_seen_when_frozen") is not False:
        raise RuntimeError("FACED design was not frozen before outcome access")
    return design


def build_args(max_iter: int) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        transfer_loss_type="dann", max_iter=max_iter,
...
def routed_snapshot(
    ...
    for start in range(0, len(data), 1024):
        ...
        parts.append(model.source_specific_predict_proba(tensor).numpy()[keep])
    selected_probability = np.concatenate(parts, axis=1).astype(np.float32)
    mixture = np.einsum(
...
def train_model(
    ...
    for iteration in range(1, trainer.max_iter + 1):
        ...
        if iteration in checkpoint_set:
            ...
            route, parts, subjects, weights = routed_snapshot(
...
def evaluate(
    ...
    best_index = min(
        range(len(rows)), key=lambda index: (-float(rows[index]["accuracy"]), index)
    )
    return {
        "study": STUDY,
        "phase": phase,
